/////////////////////////////////////////////////////////////
//   LogisticSonar 3d v.4.2 + весы  , протокол             //
...
long SumSizeMin = 1400;  //600; //минимальные измеряемые размеры, если меньше - готов к работе
...
//Канал ШИРИНА
int TR_PIN_W = 21; //A3;      //триггер одинаковый для обоих датчиков ширины
int EC_PIN_W = 20; //A2;      //echo pin on the ultrasonic sensor
long Base_W_M = 0; //базовое расстояние, введенное руками
long Base_W = 0;
long Size_W = 0;
//Канал ШИРИНА_1
//int TR_PIN_W1 = 21;   //12;     //триггер одинаковый для обоих датчиков ширины
...
void setup()
{
  ...
  if (SumBase_M == 0) { // если значения 0 - автокалибруем
    ...
    if (!lang) {
      lcd.clear(); lcd.createChar(1, bukva_I); lcd.createChar(2, bukva_Z); lcd.createChar(3, bukva_Ya); lcd.createChar(4, bukva_P);
...
{ //ручная коррекция базовых размеров
  ...
  do
  {
    ...
    //корекция значений
    if ((lcd_key == 0) && (focuspos == 0)) {Base_L = Base_L + 10; display_manualcorr (focuspos); }
    if ((lcd_key == 3) && (focuspos == 0)) {Base_L = Base_L - 10; display_manualcorr (focuspos); }
...
{ //Функция калибровки, измеряем 5 раз , находим среднее арифметическое.
 ...
 // Serial.print(" Base_H="); Serial.print(Base_H); Serial.print(" Base_W="); Serial.print(Base_W); Serial.print(" Base_L="); Serial.println(Base_L); delay(1000);
  }
...
{  // обмер по трем осям с усреднением за 3 разa 4 датчика
 ...
 for (int i=0; i < k; i++) {
          ...
          measuring (TR_PIN_W, EC_PIN_W1); Size_W1 = Size_W1 + Size; delay (50);
        ...
        Size_W1 = Size_W1/k;
        Size_W = Size_W/k;
        //Serial.print(Size_);Serial.print("-----");Serial.println(Size_W1);
...
long sizing () { 
  Size_H = 0; Size_W = 0; Size_L = 0; Size = 0; Size_W1 = 0; int k = 1; long n = 600;//n - коэффициент для устранения погрешности измерения высоты, K - коэф усреднения
  for (int i = 0; i < k; i++) {
    measuring_H (TR_PIN_H, EC_PIN_H);  Size_H = Size_H + Size; delay (50);  Serial.print (" ///H "); Serial.println (Size);
    measuring   (TR_PIN_L, EC_PIN_L);  Size_L = Size_L + Size; delay (50); 
    measuring   (TR_PIN_W, EC_PIN_W);  Size_W = Size_W + Size; delay (300); 
    measuring   (TR_PIN_W, EC_PIN_W1); Size_W1= Size_W1+ Size; delay (300);  
   } 

    if (Size_H > (Base_H - n)) {Size_L = 0; Size_W = 0; Size_H = 0;return Size_H, Size_W, Size_L; }// когда паллета убрана, высота равна базе +-n
    Size_H = Base_H - Size_H/k;
    Size_L = Base_L - Size_L/k;
    Size_W = Base_W - Size_W/k - Size_W1/k;
    //Size_W = Size_W - Size_W1/k;
    
    Serial.print (" /H "); Serial.print (Size_H); Serial.print (" /W "); Serial.print (Size_W); Serial.print (" /W_1 "); Serial.print (Size_W1); Serial.print (" /L "); Serial.println (Size_L);
...
void display_result(long delay_time_1)
{
  ...
  lcd.setCursor(0, 0); if (lang) lcd.print("  Length: ");
  if (!lang) {lcd.print("  "); lcd.write(3); lcd.write(1); lcd.write(2); lcd.print("HA  : "); }
    delay (delay_time_1); Size_L = (round(float(Size_L / ko)) * ko); lcd.print(Size_L);
  lcd.setCursor(0, 1); if (lang) lcd.print("   Width: ");
  if (!lang) {lcd.print("  "); lcd.write(6); lcd.write(2); lcd.print("P"); lcd.write(2); lcd.print("HA : "); }
    delay (delay_time_1); Size_W = (round(float(Size_W / ko)) * ko); lcd.print(Size_W);
  lcd.setCursor(0, 2); if (lang) lcd.print("  Heigth: ");
  if (!lang) {lcd.print("  B"); lcd.write(5); lcd.print("COTA : "); }
    delay (delay_time_1); Size_H = (round(float(Size_H / ko)) * ko); lcd.print(Size_H);
...
void display_ok ()   //экран режима измерения
{
  ...
  if (!lang) {lcd.setCursor(0, 3); lcd.print("  BEC    : "); lcd.print(weight); } //lcd.print(" K"); lcd.write(4); }
}
...
void display_welcome() // заставка при включении
{
  ...
  if (!lang) {
    ...
    lcd.setCursor(0, 3);  lcd.print("pallet vers. s/n 002");
    //delay (1000);
  }
  if (lang) {
