...
//обработка нажатия кнопок в режиме меню 
          ...
          if (menupos == 0) {calibration(); 
                              for (int i = 10; i < EEPROM.length(); i++) EEPROM.write(i, 0);} // очистка памяти начиная с 10-й ячейки, не трогаем 8 и 9 там тарировка? 4 - тип выводв          }
          if (menupos == 1) manual_calibration();  //Ручная Калибровка 
          if (menupos == 2) set_outputtype();  //Выбор типа вывода
...
void display_outputtype ()    //дисплей выбора типа вывода
        ...
        lcd.clear(); 
            ...
            if (!lang){
              lcd.createChar(1, bukva_I); lcd.createChar(2, bukva_P); lcd.createChar(3, bukva_D);
              lcd.createChar(4, bukva_Y); lcd.createChar(5, bukva_CH); lcd.createChar(6, bukva_IY);
              lcd.setCursor(0, 0);  lcd.print("     T"); lcd.write(1);lcd.write(2); lcd.print(" B"); lcd.write(7);lcd.print("BO");lcd.write(8); lcd.print("A");
...
void taring() {
   ...
   for (int i = 0 ; i < 2 ; i++) {
     if (i==1) scale.tare();   
          if (lang){
               lcd.clear();
               if (i==0)  {lcd.clear(); lcd.setCursor(0, 1);  lcd.print("Tare correction");
                lcd.setCursor(0, 3); lcd.print("Tare Weight: "); weighting(); lcd.print(weight, prec); lcd.print(" g");delay (1000);}  
               if (i==1)  {lcd.clear(); lcd.setCursor(0, 1);  lcd.print(" Tare correction");
                lcd.setCursor(0, 3); lcd.print(" Weight:     "); weighting(); lcd.print(weight, prec); lcd.print(" g");delay (1000);}
         }
            if (!lang){lcd.clear(); lcd.createChar(3, bukva_I); lcd.createChar(4, bukva_g); lcd.createChar(5,bukva_RR); lcd.createChar(6, bukva_g); 
              if (i==1)  {lcd.clear(); lcd.setCursor(0, 1);  lcd.print("     TAP"); lcd.write(3);lcd.print("POBKA");
                  lcd.setCursor(0, 3); lcd.print("  BEC:     "); weighting(); lcd.print(weight, prec); lcd.print(" "); lcd.write(4);  delay (1000);}  
               if (i==0)  {lcd.setCursor(0, 1);  lcd.print("     TAP"); lcd.write(3);lcd.print("POBKA");
                  lcd.setCursor(0, 3); lcd.print("  BEC TAP"); lcd.write(5); lcd.print(": "); weighting();lcd.print(weight, prec); lcd.print(" "); lcd.write(4);  delay (1000);}
            }      
     }
     //scale.power_down();  
  lcd.clear();   menu = LOW;   display_ready();//выход в меню 
}
*/ 
/*void display_scales() //отображение веса на экране готовности
{
  lcd.setCursor(10, 3); lcd.print("      ");  
        if (lang) {
            lcd.setCursor(0, 3); lcd.print(" Weight:     "); weighting_1(); lcd.print(weight,prec); lcd.print(" g");//delay (1000);}
            }         
        if (!lang) { // на русском
...
void printing () // в метрах. 0,00
       ...
       int s1, s2, s3, s4, s5, s6;
            ...
            s2 = ((Size_L/10)%10);
            s1 = ((Size_L)%10);  
     if ((s1 >= 5)&&(s2 < 9))  {s2 = s2 + 1;}
     if ((s1 >= 5)&&(s2 == 9)) {s3 = s3 + 1; s2 = 0;}
         Keyboard.print(s4);
           Keyboard.print(",");
            Keyboard.print(s3);
             Keyboard.print(s2);
